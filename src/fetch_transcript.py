...
Usage:
    python fetch_transcript.py <url_or_video_id> [--language en,pt] [--timestamps]
...
def seg_to_dict(seg) -> dict:
    """Normalize a segment — works for both dict (old API) and object (new API)."""
...
def fetch_transcript(video_id: str, languages: list = None):
    """Fetch transcript segments from YouTube. Compatible with old and new API versions."""
    try:
    ...
    # Try new API style first (instance method), fall back to old (class method)
    try:
        ytt = YouTubeTranscriptApi()
        if languages:
            raw = ytt.fetch(video_id, languages=languages)
        else:
            # Try without language first; if it fails, list available and retry
            try:
                raw = ytt.fetch(video_id)
            except Exception:
                transcript_list = ytt.list(video_id)
                transcript = transcript_list.find_manually_created_transcript([]) if hasattr(transcript_list, 'find_manually_created_transcript') else None
                if transcript is None:
                    # Pick first generated transcript available
    ...
    return [seg_to_dict(s) for s in raw]
...
def main():
    ...
    parser.add_argument("--language", "-l", default=None,
                        help="Comma-separated language codes (e.g. en,pt). Default: auto-detect")
    ...
    try:
        ...
        elif "no transcript" in error_msg.lower() or "NoTranscriptFound" in error_msg:
            print(json.dumps({"error": f"No transcript found. Try specifying a language with --language pt or --language en"}))
        else:
